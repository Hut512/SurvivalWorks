    ...
    public Device(PhysicalDevice physicalDevice){
        ...
        try(MemoryStack stack = MemoryStack.stackPush()) {
            ...
            for (int i = 0; i < queueBuf.capacity(); i++) {
                ...
                createInfoBuf.get(i).sType$Default().queueFamilyIndex(i).pQueuePriorities(prio);
            }
            ...
            VkDeviceCreateInfo deviceCreateInfo = VkDeviceCreateInfo.calloc(stack).sType$Default().ppEnabledExtensionNames(reqExt).pEnabledFeatures(features).pQueueCreateInfos(createInfoBuf);
            PointerBuffer pp = stack.mallocPointer(1);
            Util.check(vkCreateDevice(physicalDevice.getPhysicalDevice(),deviceCreateInfo,null,pp),"Could not create device");
